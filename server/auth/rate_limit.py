...
RATE_LIMIT_DURATION = timedelta(
    minutes=int(get_env_variable("RATE_LIMIT_DURATION") or 1)
)


async def verify_rate_limit(
    user_id: Optional[str] = Depends(get_user_id),
    auth_client: BaseAuthClient = Depends(get_auth_client),
):
    ...
    rows = (
        table.select("id, user_id, last_request, request_count")
        .eq("user_id", user_id)
        .execute()
    )
    ...
    user_usage = (
        rows.data[0]
        if len(rows.data) > 0
        else {"user_id": user_id, "request_count": 0, "last_request": now}
    )
